...
| `add(item: T)` | `List<T>` | `List<T>` | Mutates in place, returns `self` (chainable) |
...
| `map(fn: T -> U)` | `Set<T>` | `Set<U>` | Transforms each element with `fn` |
| `filter(fn: T -> Bool)` | `Set<T>` | `Set<T>` | Keeps elements for which `fn` returns `true` |
