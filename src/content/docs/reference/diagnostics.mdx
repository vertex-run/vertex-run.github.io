...
### `E165`: `Option`, `Result`, or tuple in a `global` signature

`Option<T>`, `Result<T, E>`, and tuples emit as `public` inner classes
of the Vertex stdlib helper class and cannot cross namespace
boundaries. They are forbidden in any `global` signature position.

**To fix:** unwrap the value at the boundary, or define a user-level
`global type` that models the same shape.
