...
`Database.update`, `Database.delete`, and `Database.undelete` require the SObject
argument to have a Salesforce Id set (i.e. `Id: Some(record_id)` in the constructor).
Calling them on a record with no Id fails on the org.

In JIT mode, DML operations are simulated: `Database.insert` returns a stub `Id` value
with an incrementing counter, and all operations print a notice to stderr.
