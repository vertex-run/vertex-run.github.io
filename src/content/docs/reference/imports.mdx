...
root, and the source directory under it ([`src` in `vertex.jsonc`](/tooling/config/),
default `src/`) is the module tree. Only the transitively
...
Only `pub` and `global` declarations are importable. Calling a private
symbol from another module is a compile error (`E124`). For the full
three-tier access model (private, `pub`, `global`), the Apex-class
propagation rules, and the strict-propagation constraints on `global`
signatures, see [Visibility](/reference/visibility/).
