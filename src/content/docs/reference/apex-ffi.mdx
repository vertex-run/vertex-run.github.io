...
```vertex
fn configurePolicy(mutable let msg: Messaging.SingleEmailMessage): Void {
    msg.optOutPolicy = "SEND_OPT_OUT"   // ok — receiver is mutable
    Void
}
```
...
Some Apex APIs — `Database.queryWithBindings`, `JSON.deserializeUntyped`,
parts of `Schema.describe*` — take or return Apex's universal `Object`
type. See [`Apex.Object`](/reference/apex-object/) for the interop escape
hatch that makes those APIs reachable without breaking Vertex's nominal
type system.
