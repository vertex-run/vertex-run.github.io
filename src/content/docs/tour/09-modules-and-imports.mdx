...
Declarations are private by default. Use `pub` to export to other modules
in the same Salesforce namespace, and `global` for the cross-namespace
tier required by `@RestResource`, `@InvocableMethod`, and managed-package
APIs:
...
global fn ping(): String { "pong" }                    // cross-namespace
```

See the [Visibility reference](/reference/visibility/) for the full
access model and the strict rules Apex enforces on `global` signatures.
...
- [Visibility reference](/reference/visibility/): the three-tier access model
- [Constants reference](/reference/bindings/#constants), `pub const` for exported values
