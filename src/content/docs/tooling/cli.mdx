...
description: Command reference for vertex run, vertex test, vertex build, vertex check, vertex init, vertex docs, and vertex lsp.
sidebar:
  order: 2
---
...
| `init`  | Scaffold a `vertex.jsonc` with recommended defaults       |
...
project's source directory ([`src` in `vertex.jsonc`](/tooling/config/),
default `src/`) as the module root:
...
`*_test.vtx` file under the source directory named by
[`vertex.jsonc`](/tooling/config/) and runs them all.
...
class files. Source, output, and clean behavior are configured in
[`vertex.jsonc`](/tooling/config/) at the project root. A config file
is required; run `vertex init` to scaffold one.
...
When no config file is present, `vertex build` exits 1 with a
hand-holding error pointing at `vertex init`.
...
Scaffolds a starter `vertex.jsonc` in the current working directory
with recommended defaults and inline comments. Refuses to overwrite an
existing `vertex.jsonc` or `vertex.json`. Prints a non-fatal note if
`sfdx-project.json` is missing.

See [Config file](/tooling/config/) for the full schema.
