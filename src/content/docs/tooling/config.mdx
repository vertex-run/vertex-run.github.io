...
The other commands do not require it — `vertex run`, `vertex test`,
`vertex check` and the language server fall back to the default `src`
directory when a project has no config file. When the file *is* present,
every command reads `src` from it.
...
`src` governs every command, not just the build. `vertex run` resolves
imports against it, `vertex test` discovers `*_test.vtx` files under it,
`vertex check` resolves modules through it, and the language server roots
its cross-file index there.
