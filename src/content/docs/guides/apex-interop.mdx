...
`pub fn` declarations become `public static` methods on the generated
...
```apex
String greeting = Greet.say_hello('Alice');
```

This is how a team adopts Vertex incrementally: new code is Vertex;
existing Apex callers reach in through the generated API. Because the
names carry over unchanged, replacing an Apex class with a Vertex module
of the same name leaves its callers untouched.

<Aside type="note">
A name is rewritten only when Apex would reject it: an identifier that
collides with an Apex reserved word, or one that starts with `_`, is
prefixed with `vtx_`. So `pub fn run()` becomes `vtx_run()` — `run` is
reserved — while `say_hello` stays `say_hello`. See
[Apex class names](/tooling/project-structure/#apex-class-and-method-names).
</Aside>
